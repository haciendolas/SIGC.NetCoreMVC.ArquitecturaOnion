...
{
    internal class UserUpdateCommandHandler : IRequestHandler<UserUpdateCommandRequest, MsgResponse<object?>>
    {
        ...
        private readonly IUserCompanyUpdateRepository UserCompanyUpdateRepository;
        private readonly IUserRoleCreateRepository UserRoleCreateRepository;
        private readonly IUserRoleDeleteRepository UserRoleDeleteRepository;       
        ...
        public UserUpdateCommandHandler(
            ...
            IUserCompanyUpdateRepository UserCompanyUpdateRepository,
            IUserRoleCreateRepository UserRoleCreateRepository,
            IUserRoleDeleteRepository UserRoleDeleteRepository
        )
        {
            ...
            this.UserCompanyUpdateRepository = UserCompanyUpdateRepository;
            this.UserRoleCreateRepository = UserRoleCreateRepository;
            this.UserRoleDeleteRepository = UserRoleDeleteRepository;
        }

        public async Task<MsgResponse<object?>> Handle(UserUpdateCommandRequest Request, CancellationToken CancellationToken)
        {
            ...
            try
            {
                var Model = User.Update( Request.UserID,
                                        Request.UserFirstName,
                                         ...
                                         Request.UserMail,
                                         Request.UserPhotoBandera == "DELETE" ? null : Request.File == null ? Request.UserPhoto : $"{Request.UserName.Replace(" ", "").ToUpper().Trim()}{Path.GetExtension(Request.File.FileName)}",
                ...
                if (Verify == VerifyRegistryConst.User.OK)
                {
                    ...
                    if (RecordAffected > 0)
                    {
                        ...
                        if (Request.File is not null)
                        {
                            if (!string.IsNullOrWhiteSpace(Request.UserPhoto))
                            {
                                FileEntry.FileName = Request.UserPhoto;
                                FileEntry.FileLocation = $"{FileUploadSettings.UserPhotoLocation}/{Request.UserPhoto}";
                        ...
                        if (Request.UserPhotoBandera == "DELETE")
                        {
                            FileEntry.FileName = Request.UserPhoto;
                            FileEntry.FileLocation = $"{FileUploadSettings.UserPhotoLocation}/{Request.UserPhoto}";
                            await FileStorageService.DeleteAsync(FileEntry, CancellationToken);
