...
{
    internal class WarehouseCreateCommandHandler : IRequestHandler<WarehouseCreateCommandRequest, MsgResponse<object?>>
    {       
        private readonly ICurrentSessionService CurrentSessionService;
        private readonly IMessageService MessageService;   
        private readonly IWarehouseCreateRepository WarehouseCreateRepository;

        public WarehouseCreateCommandHandler(          
            ICurrentSessionService CurrentSessionService,
            IMessageService MessageService,
            IWarehouseCreateRepository WarehouseCreateRepository
            )
        {           
            this.CurrentSessionService = CurrentSessionService;
            this.MessageService = MessageService;           
            this.WarehouseCreateRepository = WarehouseCreateRepository;
        }
        public async Task<MsgResponse<object?>> Handle(WarehouseCreateCommandRequest Request, CancellationToken CancellationToken)
        {
            var MsgResponse = new MsgResponse<object?>();            
            try
            {
                var CurrentDate = DateTime.Now;
                var Model = Warehouse.Create(
