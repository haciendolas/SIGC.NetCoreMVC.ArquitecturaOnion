...
BEGIN
  SET NOCOUNT ON
    ...
    SELECT C.CompanyID,'' AS TaxpayerTypeName,C.CompanyDocumentNumber,C.CompanySocialReason,'' AS SectorName,
	 ...
	 WHERE CR.CompanyIDRegister=@CompanyIDRegister AND C.StateID!=2 
	   ...
	   AND C.SectorID=CASE WHEN @SectorID IS NULL OR @SectorID=0 THEN C.SectorID ELSE @SectorID END   
