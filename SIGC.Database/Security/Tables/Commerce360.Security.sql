...
GO
CREATE  TABLE [Security].Token( --TokenSession
...
GO 
CREATE TABLE [Security].TokenResetPassword(
    TokenID INT IDENTITY(1,1),                       
    CompanyID INT NOT NULL,                                      
    UserID INT NOT NULL,                                        
    TokenResetPasswordHash NVARCHAR(256) NOT NULL,                
    TokenResetPasswordCreateDateTime DATETIME2 NOT NULL, -- Fecha creación
    TokenResetPasswordExpirationDateTime DATETIME2 NOT NULL,      -- Fecha expiración
    TokenResetPasswordUsedDateTime DATETIME2 NULL,                -- Fecha de uso
    TokenResetPasswordRevocationDateTime DATETIME2 NULL,          -- Fecha revocación
    StateID AS (                                                   -- 1=activo, 0=inactivo
        CASE 
            WHEN TokenResetPasswordRevocationDateTime IS NULL
             AND TokenResetPasswordExpirationDateTime > SYSUTCDATETIME()
             AND TokenResetPasswordUsedDateTime IS NULL
            THEN 1 
            ELSE 0 
        END
    ) ,
    CONSTRAINT TokenResetPassword_PK_TokenID PRIMARY KEY(TokenID),
    CONSTRAINT TokenResetPassword_FK_UserID FOREIGN KEY(UserID) REFERENCES Security.[User](UserID)
)
GO

-- Índice recomendado para búsquedas rápidas
CREATE INDEX TokenResetPassword_IX_UserID_CompanyID_Hash
ON [Security].TokenResetPassword(UserID, CompanyID, TokenResetPasswordHash);

/*
| Tipo           | Tabla separada | Expiración | Uso                  |
| -------------- | -------------- | ---------- | -------------------- |
| JWT / sesión   | TokenSesion    | Horas/días | Renovable, revocable |
| Reset password | TokenReset     | 15–30 min  | 1 uso                |
| OTP / MFA      | UserOTPTokens  | 1–5 min    | 1 uso                |
